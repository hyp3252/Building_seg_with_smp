...
from edge_util.edge_stitch import InferDiv, InferStitch
...
edge_folder_name = '/output/pred/edge_test_buildings'
GT_FORMAT = '.png'
INPUT_SIZE = 512 # 학습으로 들어가는 영상 크기
ORIGINAL_SIZE = 1024 # 데이터셋 영상 크기 
x_window_step = 200
y_window_step = 200
BACKBONE_DIR = 'C:/Users/yonghyun/Desktop/smp_building/weight/addEdge_fourchannels_FPN_res50_resize512_AdamW/'
BACKBONE_NAME = model_mode + '_29.pth'
csv_data = 'test_list.csv'
"""------------------------ SAVE ---------------------------"""
RESULTS_DIR = './results/addEdge_fourchannels_FPN_res50_resize512_ep29_AdamW' # 저장 경로
"""------------------------ function parameters ---------------------------"""
...
#     f = open(args.data_dir + args.list_file_name, 'r')
    ...
    for cntList in range(name_list.__len__()):
        ...
        edge_file = args.data_dir + "/" + edge_folder_name + "/%s" % name_list[cntList]
        img_rgb = Image.open(img_file).convert('RGB') 
        img_edge = Image.open(edge_file).convert('L')
        ...
        print("img orig + img edge shape ", img_combined.shape)
        ...
        imgset, num_patches = divided_img.forward(img_combined, IMG_MEAN)

        for cnt_patches in range(num_patches):
            onepatch = imgset[cnt_patches]['divided_img']
            onepatch = onepatch.expand(1, img_combined.shape[-1], INPUT_SIZE_m[0], INPUT_SIZE_m[1])
            if cnt_patches == 0:
                sets = onepatch
            else:
    ...
    print(f"Add Edge {model_mode} Test results")
    print("=======================================")
