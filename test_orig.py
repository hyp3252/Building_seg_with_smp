...
"""==============================================================================="""
gpu_device = '0' # GPU 번호
model_mode = 'FPN_res50' # Unet_res101 / Unet_res50 / Unetpp_res101 / Unetpp_res50 / FPN_res101 / FPN_res50 / DeeplabV3p_res101 / DeeplabV3p_res50
"""=============================================================================="""
"""                                                                              """
"""                             주요 파라미터 셋팅                                 """
"""                                                                              """
...
"""------------------------ LOAD ---------------------------"""
DATA_DIR = 'C:/Users/yonghyun/Desktop/buildingdetection2020-main'
img_folder_name = '/buildings/Test/test_buildings_data/test_buildings_data'
label_folder_name = '/label_color_contour_test' #'trainLabel' #'GT_rgbseg'
GT_FORMAT = '.png'
INPUT_SIZE = 512 # 학습으로 들어가는 영상 크기
ORIGINAL_SIZE = 1024 # 데이터셋 영상 크기 
x_window_step = 200
y_window_step = 200
BACKBONE_DIR = 'C:/Users/yonghyun/Desktop/smp_building/weight/original_building_FPN_res50_resize512_AdamW/'
BACKBONE_NAME = model_mode + '_29.pth'
csv_data = 'test_list.csv'
"""------------------------ SAVE ---------------------------"""
RESULTS_DIR = './results/original_building_FPN_res50_resize512_ep29_AdamW' # 저장 경로
"""------------------------ function parameters ---------------------------"""
NUM_CLASSES = 7  # 0:배경, 1:소형, 2:아파트, 3:공장, 4:중형단독시설, 5:대형시설, 6:contour
trans_ratio = 0.5 # save image + input predict 투영율
...
def main():
    ...
    if args.model_mode == 'Unet_res101':
        model = smp.Unet(encoder_name="resnet101", in_channels=3, classes=7)
    elif args.model_mode == 'Unet_res50':
        model = smp.Unet(encoder_name="resnet50", in_channels=3, classes=7)
    elif args.model_mode == 'Unetpp_res101':
        model = smp.UnetPlusPlus(encoder_name="resnet101", in_channels=3, classes=7)
    elif args.model_mode == 'Unetpp_res50':
        model = smp.UnetPlusPlus(encoder_name="resnet50", in_channels=3, classes=7)
    elif args.model_mode == "FPN_res101":
        model = smp.FPN(encoder_name="resnet101", in_channels=3, classes=7)
    elif args.model_mode == "FPN_res50":
        model = smp.FPN(encoder_name="resnet50", in_channels=3, classes=7)
    elif args.model_mode == "DeeplabV3p_res101":
        model = smp.DeepLabV3Plus(encoder_name="resnet101", in_channels=3, classes=7)
    elif args.model_mode == "DeeplabV3p_res50":
        model = smp.DeepLabV3Plus(encoder_name="resnet50", in_channels=3, classes=7)
    else:
    ...
    """-------------------------- FILE SAVE --------------------------"""
    if not os.path.exists(args.results_dir + '_featuremap/'):
        os.makedirs(args.results_dir + '_featuremap/')
...
#     f = open(args.data_dir + args.list_file_name, 'r')
    ...
    for cntList in range(name_list.__len__()):
        ...
        label_building = Image.open(label_file)
        # label_building = Image.open(label_file)
        ...
        imgset, num_patches = divided_img.forward(img_rgb, IMG_MEAN)
        ...
        ''' 모델 연산 '''
        with torch.no_grad():
            pred_model = model(image_input.float())
        ...
        label_load = 'C:/Users/yonghyun/Desktop/buildingdetection2020-main/buildings/Test/test_buildings_labeling/test_buildings_labeling' +"/%s" % name_list[cntList]
        label_save = Image.open(label_load)
        label_save = np.asarray(label_save, np.float32)

        plt.imsave(args.results_dir + '_featuremap/' +'GT_' + name_list[cntList] , label_save/255)
        plt.imsave(args.results_dir + '_featuremap/' +'FM_' + name_list[cntList] , merged_results)
    ...
    print(f"Original {model_mode} Test results")
    print("=======================================")
