...
class InferDiv:
    def __init__(self, size_sample=[225, 255], original_size=[900, 900], x_window_step=5, y_window_step=5):
        ...
        self.x_window_num = int(np.ceil(original_size[0] / x_window_step)) # width, 1024/5 = 205
        self.y_window_num = int(np.ceil(original_size[1] / y_window_step)) # height, 1024/5 = 205
    ...
    def forward(self, img, IMG_MEAN):
        ...
        if img.shape[2] == 4 and len(IMG_MEAN) == 3:
            img[:,:,:3] = (img[:,:,:3] - IMG_MEAN) / 128
        else:
...
class InferStitch:
    ...
    def forward(self, div_img, img_set, num_classes=2):
        w, h = self.size_sample # 512,512
        W, H = self.size_output # 1024,1024
